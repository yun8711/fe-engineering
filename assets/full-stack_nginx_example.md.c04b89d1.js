import{_ as s,v as n,b as a,R as l}from"./chunks/framework.8277b2e6.js";const p="/fe-engineering/assets/nginx02.30799f1a.png",e="/fe-engineering/assets/nginx03.482b4c71.png",o="/fe-engineering/assets/nginx04.80fa0969.png",g=JSON.parse('{"title":"Nginx 配置示例","description":"","frontmatter":{"outline":"deep"},"headers":[],"relativePath":"full-stack/nginx/example.md","filePath":"full-stack/nginx/example.md","lastUpdated":1710644343000}'),t={name:"full-stack/nginx/example.md"},c=l(`<h1 id="nginx-配置示例" tabindex="-1">Nginx 配置示例 <a class="header-anchor" href="#nginx-配置示例" aria-label="Permalink to &quot;Nginx 配置示例&quot;">​</a></h1><h2 id="vue-router-的-history-模式" tabindex="-1">vue-router 的 history 模式 <a class="header-anchor" href="#vue-router-的-history-模式" aria-label="Permalink to &quot;vue-router 的 history 模式&quot;">​</a></h2><p>vue-router 的 history 模式，使用浏览器的 history API来管理路由状态，而不是依赖于URL中的哈希，但是由于Vue是单页SPA应用，所以每个路由并没有对应的html文件。</p><p>所以配置 history 模式的关键就在于，当 url 路径对应的文件或目录不存在时，重定向到 index.html，使得所有的路由请求都指向了 Vue 应用的入口页面，这样就能正确的处理动态路由。</p><div class="language-ini"><button title="Copy Code" class="copy"></button><span class="lang">ini</span><pre class="shiki material-theme-palenight"><code><span class="line"><span style="color:#BABED8;">location / {</span></span>
<span class="line"><span style="color:#BABED8;">  try_files $uri $uri/ /index.html</span><span style="color:#676E95;font-style:italic;">;</span></span>
<span class="line"><span style="color:#BABED8;">}</span></span></code></pre></div><h2 id="正向代理" tabindex="-1">正向代理 <a class="header-anchor" href="#正向代理" aria-label="Permalink to &quot;正向代理&quot;">​</a></h2><p>一般的访问流程是客户端直接向目标服务器发送请求并获取内容，使用正向代理后，客户端改为向代理服务器发送请求，并指定目标服务器（原始服务器），然后由代理服务器和原始服务器通信，转交请求并获得内容，再返回给客户端。</p><p>正向代理的<strong>代理对象是客户端</strong>，即替客户端去访问目标服务器，使真实客户端对服务器不可见；</p><p>比如 VPN，你的浏览器无法直接访问谷歌，这时候可以通过一个代理服务器来帮助你访问谷哥，那么这个服务器就叫正向代理。</p><p><img src="`+p+`" alt="正向代理原理图"></p><p><em>示例</em></p><div class="language-ini"><button title="Copy Code" class="copy"></button><span class="lang">ini</span><pre class="shiki material-theme-palenight"><code><span class="line"><span style="color:#BABED8;">server {</span></span>
...
<span class="line"><span style="color:#89DDFF;font-style:italic;">fi</span></span></code></pre></div><p>4、启动两台服务器的 nginx 和 keepalived</p><h2 id="websocket" tabindex="-1">WebSocket <a class="header-anchor" href="#websocket" aria-label="Permalink to &quot;WebSocket&quot;">​</a></h2><p>WebSocket是HTML5下一种新的协议。实现了浏览器与服务器全双工通信，能更好的节省服务器资源和带宽并达到实时通讯的目的。</p><p>它与HTTP一样通过已建立的TCP连接来传输数据，但是它和HTTP最大不同是：</p><ul><li>WebSocket是一种双向通信协议。在建立连接后，WebSocket 服务器端和客户端都能主动向对方发送或接收数据</li><li>WebSocket需要像TCP一样，先建立连接，连接成功后才能相互通信</li></ul><p>WebSocket应用程序可以在客户端和服务器之间保持长时间运行的连接，从而有助于开发实时应用程序。Nginx从 v1.3 版本开始支持WebSocket，其可以作为一个反向代理和为WebSocket程序做负载均衡。</p><p><em>示例</em></p><div class="language-ini"><button title="Copy Code" class="copy"></button><span class="lang">ini</span><pre class="shiki material-theme-palenight"><code><span class="line"><span style="color:#BABED8;">user  nginx</span><span style="color:#676E95;font-style:italic;">;</span></span>
<span class="line"><span style="color:#BABED8;">worker_processes  auto</span><span style="color:#676E95;font-style:italic;">;</span></span>
<span class="line"></span>
<span class="line"><span style="color:#BABED8;">error_log  /var/log/nginx/error.log notice</span><span style="color:#676E95;font-style:italic;">;</span></span>
<span class="line"><span style="color:#BABED8;">pid        /var/run/nginx.pid</span><span style="color:#676E95;font-style:italic;">;</span></span>
<span class="line"></span>
<span class="line"></span>
<span class="line"><span style="color:#BABED8;">events {</span></span>
<span class="line"><span style="color:#BABED8;">    worker_connections  1024</span><span style="color:#676E95;font-style:italic;">;</span></span>
<span class="line"><span style="color:#BABED8;">}</span></span>
<span class="line"></span>
<span class="line"><span style="color:#BABED8;">http {</span></span>
<span class="line"><span style="color:#BABED8;">    map $http_upgrade $connection_upgrade {</span></span>
<span class="line"><span style="color:#BABED8;">        default upgrade</span><span style="color:#676E95;font-style:italic;">;</span></span>
<span class="line"><span style="color:#BABED8;">        </span><span style="color:#89DDFF;">&#39;&#39;</span><span style="color:#BABED8;"> close</span><span style="color:#676E95;font-style:italic;">;</span></span>
<span class="line"><span style="color:#BABED8;">    }</span></span>
<span class="line"><span style="color:#BABED8;"> </span></span>
<span class="line"><span style="color:#BABED8;">    upstream websocket { </span></span>
<span class="line"><span style="color:#89DDFF;">    		</span><span style="color:#676E95;font-style:italic;"># 代理地址</span></span>
<span class="line"><span style="color:#BABED8;">        server 121.42.165.18:8800</span><span style="color:#676E95;font-style:italic;">;</span></span>
<span class="line"><span style="color:#BABED8;">    }</span></span>
<span class="line"><span style="color:#BABED8;"> </span></span>
<span class="line"><span style="color:#BABED8;">    server {</span></span>
<span class="line"><span style="color:#BABED8;">        listen 8021</span><span style="color:#676E95;font-style:italic;">;</span></span>
<span class="line"><span style="color:#BABED8;">        location / {</span></span>
<span class="line"><span style="color:#89DDFF;">        		</span><span style="color:#676E95;font-style:italic;"># 代理上面的websocket，也可以直接写121.42.165.18:8800</span></span>
<span class="line"><span style="color:#BABED8;">            proxy_pass http://websocket</span><span style="color:#676E95;font-style:italic;">;	</span></span>
<span class="line"><span style="color:#BABED8;">            proxy_http_version 1.1</span><span style="color:#676E95;font-style:italic;">;</span></span>
<span class="line"><span style="color:#89DDFF;">            </span><span style="color:#676E95;font-style:italic;"># 表示设置Upgrade不变</span></span>
<span class="line"><span style="color:#BABED8;">            proxy_set_header Upgrade $http_upgrade</span><span style="color:#676E95;font-style:italic;">;</span></span>
<span class="line"><span style="color:#89DDFF;">            </span><span style="color:#676E95;font-style:italic;"># 表示如果 $http_upgrade为upgrade，则请求为upgrade(websocket)，如果不是，就关闭连接</span></span>
<span class="line"><span style="color:#BABED8;">            proxy_set_header Connection $connection_upgrade</span><span style="color:#676E95;font-style:italic;">;</span></span>
<span class="line"><span style="color:#BABED8;">            proxy_set_header Host $host</span><span style="color:#676E95;font-style:italic;">;</span></span>
<span class="line"><span style="color:#BABED8;">        }</span></span>
<span class="line"><span style="color:#BABED8;">    }</span></span>
<span class="line"><span style="color:#BABED8;">}</span></span></code></pre></div><p>说明：</p><p>1、map指令</p><p>根据客户端请求中的值，来构造改变connection_upgrade的值，即根据变量的值创建新的变量connection_upgrade，创建的规则就是<code>{}</code>里面的东西。</p><p>其中的规则没有做匹配，因此使用默认的，即 :</p><ul><li>如果<code>$http_upgrade 不为 &#39;&#39; (空)，</code> 则$connection_upgrade 为 upgrade 。</li><li>如果<code>$http_upgrade 为 &#39;&#39; (空)，</code> 则 $connection_upgrade 为 close。</li></ul><p>2、默认情况下，如果代理服务器在 60 秒内没有传输任何数据，连接将被关闭。可以使用proxy_read_timeout指令增加此超时 。</p><h2 id="http2" tabindex="-1">HTTP2 <a class="header-anchor" href="#http2" aria-label="Permalink to &quot;HTTP2&quot;">​</a></h2><p>HTTP 全称是Hypertext Transfer Protocol，是在1989年World Wide Web发展起来之后出现的标准协议，用来在WWW上传输数据。<code>HTTP/1.1</code>是1997年在原始的HTTP协议基础上进行的补充和优化。</p><p>到了2015年，为了适应快速发送的web应用和现代浏览器的需求，发展出了新的<code>HTTP/2</code>协议，主要在<code>手机浏览器</code>、<code>延时处理</code>、<code>图像处理</code>和<code>视频处理</code>方面进行了优化。</p><p>相对于HTTP1.1来说，HTTP2有如下几个优点：</p><ul><li>使用<strong>多路复用</strong>技术，在同一个连接中可以并行处理多个请求。</li><li>可以压缩HTTP头，减少请求的大小。</li><li>数据传输格式是以二进制进行的，所以传输更加有效。</li><li>服务器可以向客户端推送数据，从而让应用程序可以处理更加复杂的功能。</li></ul><p>尽管HTTP2 并不要求使用加密，但是对于现代浏览器来说如 Google Chrome 和 Mozilla Firefox默认HTTP2 和HTTPS 是一起使用的，所以如果想配置HTTP2的话，还需要同时配置SSL。</p>`,86),i=[c];function r(y,B,D,E,d,h){return n(),a("div",null,i)}const u=s(t,[["render",r]]);export{g as __pageData,u as default};
