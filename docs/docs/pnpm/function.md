...
一个 workspace 的根目录下必须有 [`pnpm-workspace.yaml`](https://pnpm.io/zh/pnpm-workspace_yaml) 文件，它是用来定义 pnpm 工作空间（workspace）的配置文件，书写格式遵循 [YAML](../../guide/text-format) 格式规范，配置项有：
